    ...
    <>
      {items && items.length && (
        <div className={twMerge('', containerClass)}>
          {items.map(({ title, step, description, icon: Icon }, index = 0) => (
            <div key={id ? `item-${id}-${index}` : `item-grid-${index}`} className={twMerge('flex', panelClass)}>
              <div className="flex flex-col items-center mr-4 rtl:mr-0 rtl:ml-4">
                <div
                  className={`flex items-center justify-center ${index !== items.length - 1 ? '' : 'text-slate-200 rounded-full bg-primary-900'
                    }`}
                >
                  {Icon ? (
              ...
              <div className={`pt-1 ${index !== items.length - 1 ? 'pb-8' : ''}`}>
                ...
                {description && (
                  <p className={twMerge('text-gray-600 dark:text-slate-400', descriptionClass)}>
                    <span style={{ fontWeight: 'bold', color: "red" }}> {step} </span>
                    {' : '} {description}
                  </p>
                )}
              </div>
